    ...
    <div className="rounded shadow-lg relative bg-white m-4 transition-transform transform hover:scale-95 pb-5 space-y-3 ">
      ...
      <div className="flex justify-left items-center pt-2 flex-wrap">
        <div className="px-6 p-2">
          {
            token ? (
              <Link to={`/courses/${courseDetails._id}`} className="bg-blue-500 text-white font-bold py-2 px-4 rounded">Explore</Link>
            ) : (
              <button className="bg-blue-500 text-white font-bold py-2 px-4 rounded" onClick={handleShowcourseDetails}>Explore</button>
            )
          }
        </div>
